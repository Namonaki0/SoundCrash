...
const recordWrap = document.querySelectorAll(".side-view-img-wrap");
...
function recordMenuChoice({
  ...
  recordWrap.forEach((each_record) => {
    each_record.addEventListener("click", async (e) => {
      const dataSet = await e.target.dataset.value;
      const title = document.querySelector(".inner-wrap h2");
      const recordCover = document.querySelector(".img-wrap img");
